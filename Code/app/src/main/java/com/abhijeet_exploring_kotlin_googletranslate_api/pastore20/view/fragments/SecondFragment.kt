...
//        msg = view.findViewById(R.id.tvMsg)
    ...
    private fun functionality(view: View) {
        ...
        btnAdd.setOnClickListener {
            ...
            var desc : String? = view.findViewById<EditText>(R.id.etDescription).text.toString().trim()

            if(common.validateIp(title, password, rePassword)){
                try {
                    if(desc == "") desc = null
                    val oneAccount = OneAccount(siteName = title, sitePassword = password, siteDescription = desc)
